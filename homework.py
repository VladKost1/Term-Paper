...
class RC2():
    ...
    def encode(self, input_buffer, mode, IV=None, padding=None):

        if (len(input_buffer) % RC2_SIZE) == 0 and padding != PADDING_PKCS5:
            crypt_size = len(input_buffer)
        else:
            crypt_size = ((len(input_buffer) // RC2_SIZE) + 1) * RC2_SIZE
        ...
        for i in range(crypt_size):
            if len(input_buffer) > i:
                ...
                crypt_buffer[i] = (RC2_SIZE - (len(input_buffer) % RC2_SIZE)) & 0xFF

        result = bytearray()

        for block_counter in range(crypt_size // RC2_SIZE):

            block = crypt_buffer[block_counter * RC2_SIZE:block_counter * RC2_SIZE + RC2_SIZE]

            if block_counter == 0:
                if mode == MODE_OFB and IV is not None:
                    for i in range(RC2_SIZE):
    ...
    def decode(self, input_buffer, mode, IV=None, padding=None):
        ...
        for block_counter in range(crypt_size // RC2_SIZE):
            ...
            if mode == MODE_OFB:
                if block_counter == 0:
                    if IV is not None:
                        for i in range(RC2_SIZE):
                            crypt_buffer[block_counter * RC2_SIZE + i] = block_result[i] ^ IV[i]
                else:
                    for i in range(RC2_SIZE):
                        crypt_buffer[block_counter * RC2_SIZE + i] = block_result[i] ^ previous_block[i]
            else:
                for i in range(RC2_SIZE):
                    crypt_buffer[block_counter * RC2_SIZE + i] = block_result[i]
...
def main():
    ...
    first = open(input('Create file for your message -->'), 'w+')
    text = ' '
    for line in first:
        text += line.replace('\n', '')
    lent = bytearray(text, 'utf-8')
    out1 = rc2.encode(lent, MODE_OFB, bytearray('12345678', 'ascii'), PADDING_PKCS5)
    first.close()
    first = open(input('Choose file for your encoded message-->'), 'w')
    print(out1)
    first.write(str(out1))
    first.close()

    first = open(input('Choose file for your decoded message-->'), 'r')
    text = ''
    for line in first:
        text += line.replace('\n', '')
    first.close()
    legh = bytearray(text, 'ascii')
    legh = legh[12:-2]
    out2 = rc2.decode(out1, MODE_OFB, bytearray('12345678', 'ascii'), PADDING_PKCS5)
    first = open(input('Choose file for your message -->'), 'w')
    first.write(str(out2))
    print(out2)
    first.close()
